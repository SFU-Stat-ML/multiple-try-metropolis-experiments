...
# Purpose: MTM with homogeneous full proposals
# Author:  Renny Doig and Liangliang Wang
# Date:    January 11, 2025

function basic_mtm(runtime::Float64, M::Integer, data, target::Function, init::Function, weight::String, seed::Integer)
	Random.seed!(seed)
	...
	init_size = curr_size = Int(floor(runtime / (t0*M)))
	...
	w(y,x) = nothing # no fucking clue why, but this needs to be here
	# define log-weight function
	if weight == "proportional"
		w(y, x) = target(data, y)
	elseif weight == "importance"
		w(y, x) = target(data, y) - logpdf(MvNormal(x, adapt_scale/d*prop_cov), y)
	elseif weight == "locally balanced"
		w(y, x) = target(data, y) / 2
	elseif weight == "jump distance"
		w(y, x) = target(data, y) + 2.9/2*log(sum((y .- x).^2))
	else
	...
	while time() - start_time <= runtime
		...
		fwd_prop = MvNormal(X[n-1,:], adapt_scale/d*prop_cov)
		Threads.@threads for m in 1:M
			y_star[m] = rand(fwd_prop)
			w_y[m] = w(y_star[m], X[n-1,:])
		end
		...
		# sample from the candidates based on their weights
		J = sample(pweights(W))

		# Generate reverse samples
		bwd_prop = MvNormal(y_star[J], adapt_scale/d*prop_cov)
				
		x_star = Vector{Vector{Float64}}(undef, M)
		w_x = Vector{Float64}(undef, M)
		Threads.@threads for m in 1:M
			x_star[m] = m==J ? X[n-1,:] : rand(bwd_prop)
			w_x[m] = w(x_star[m], y_star[J]) 
		end
		...
		if n > n0
			...
			prop_cov += learn_rate(n) * ((X[n,:]-prop_mean) * (X[n,:]-prop_mean)' - prop_cov)
			# update proposal mean
			prop_mean += learn_rate(n) * (X[n,:] - prop_mean)
			...
			if !isposdef(adapt_scale/d*prop_cov)
				prop_cov = nearPD(adapt_scale/d*prop_cov) ./ (adapt_scale/d)
			end

			# and finally, make sure it's Hermitian
			prop_cov = Hermitian(prop_cov)
		end
